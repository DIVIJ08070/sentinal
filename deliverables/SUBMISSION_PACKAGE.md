...
|---|---|---|---|---|
| 1 | Presentation | `deliverables/SENTINEL_Presentation.pptx` (source: `docs/deck/build_deck.py`, screenshots in `docs/deck/shots/`) | **Ready** (12 slides, score-sheet-mirrored; participant identity on title slide + every footer; honest captions on simulated-harness screenshots) | Export a portal-safe **PDF copy** — no `soffice` on this machine: use PowerPoint/Keynote → Export as PDF, or Google Slides (import PPTX → File → Download → PDF). Verify fonts/screenshots survived. |
| 2 | High-Level Design (HLD) document | `deliverables/SENTINEL_HLD.pdf` (12 pages, all 3 mermaid diagrams **rendered as vector diagrams**, generated from `docs/HLD.md` via `scripts/export-hld-pdf.py`) | **Ready** (participant identity in the title area; PDF Title metadata set properly) | After any HLD edit: re-run `.venv/bin/python scripts/export-hld-pdf.py` → `cp docs/HLD.pdf deliverables/SENTINEL_HLD.pdf`. Eyeball all 3 diagrams + page breaks once more after any edit. |
| 3 | Demo **Video A** — own feed, 2–3 min | To record → save as `deliverables/SENTINEL_OwnFeed_Demo_DivijPatel.mp4` | **Not recorded** — and its prerequisite footage does not exist yet: film TWO spots per `deliverables/VIDEO_SHOT_SCRIPTS.md` P1 (`/Users/mac/Desktop/own_feed/traffic_a.mp4` + `traffic_b.mp4`) | Record per `deliverables/VIDEO_SHOT_SCRIPTS.md` VIDEO A (dual-camera onboarding → live ANPR → alert → **two-camera route** with the implied-speed chip → dossier). 1080p+, cursor visible, voiceover or captions, MP4 (H.264), length verified 2–3 min. Upload (Section 2) and paste the **link** into the portal form. |
| 4a | Demo **Video B** — government feed | To record → save as `deliverables/SENTINEL_GovFeed_Demo_DivijPatel.mp4` | **Not recorded** (daylight IST slot needed) | Record per `deliverables/VIDEO_SHOT_SCRIPTS.md` VIDEO B: catalogue sync live, `worker.py --detector anpr --cameras 6,23` (measured health heartbeats on camera), real detections on map/alerts, watchlist alert + the `CMCI801` route with fuzzy recoveries. Close on the output report. Upload (Section 2), paste **link**. |
